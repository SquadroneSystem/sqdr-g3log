        ...
        node(label) {
            try {
                ...
                stage('Environment Initialization') {
                    ...
                    env.setProperty('ACCESS_TOKEN', "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")
                    stage('Notify GitHub build start') {
                        sh "ci/notify_github $GIT_COMMIT $ACCESS_TOKEN $BUILD_URL pending"
                    }
                ...
                stage('Notify result') {
                    ...
                    if (currentResult == 'SUCCESS') {
                        ...
                        sh "ci/notify_github $GIT_COMMIT $ACCESS_TOKEN $BUILD_URL success"
                    } else {
                        echo "Failed to build deb packages"
                        sh "ci/notify_github $GIT_COMMIT $ACCESS_TOKEN $BUILD_URL failure"
                    }
